...
void	player_position(char *line, bool *player)
{
	int	i;
...
void	check_wall(t_data *data)
{
	...
	while (data->map[++i] != NULL)
		player_position(data->map[i], &player);
	j = -1;
...
void	check_valid_map(char *file, t_data *data)
{
	...
	while (1)
	{
		...
		if (data && data->file)
		{
			while (data->file[i] && data->file[i][j] && (data->file[i][j] == ' '
					|| data->file[i][j] == '\t'))
				j++;
			if (data->file[i] && data->file[i][j] && data->file[i][j] == '1')
			{
