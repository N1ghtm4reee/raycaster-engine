...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/03/12 01:45:50 by aakhrif           #+#    #+#             */
/*   Updated: 2025/03/12 07:29:41 by aakhrif          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
		...
		, int wall_height)
{
	...
	while (var.y < y_start + wall_height)
	{
		...
		if (found_wall(game, game->wall_hit_x, game->wall_hit_y))
			...
			var.pixel_index);
		if (found_door(game, game->wall_hit_x, game->wall_hit_y))
			var.color = *(int *)(game->door.tex_data + \
			var.pixel_index);
